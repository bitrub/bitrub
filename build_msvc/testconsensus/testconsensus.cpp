...
// bitrub includes.
#include <..\src\script\bitrubconsensus.h>
...
int main()
{
    std::cout << "bitrubconsensus version: " << bitrubconsensus_version() << std::endl;
    ...
    auto op0Result = bitrubconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, bitrubconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
