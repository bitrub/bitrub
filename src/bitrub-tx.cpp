...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " bitrub-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  bitrub-tx [options] <hex-tx> [commands]  Update hex-encoded bitrub transaction\n" +
            "or:     bitrub-tx [options] -create [commands]   Create hex-encoded bitrub transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitrub transaction
