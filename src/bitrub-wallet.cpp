// Copyright (c) 2016-2018 The BitRub Core developers
...
static bool WalletAppInit(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        std::string usage = strprintf("%s bitrub-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
                                      "bitrub-wallet is an offline tool for creating and interacting with BitRub Core wallet files.\n" +
                                      "By default bitrub-wallet will act on wallets in the default mainnet wallet directory in the datadir.\n" +
...
int main(int argc, char* argv[])
{
    ...
    if (method.empty()) {
        tfm::format(std::cerr, "No method provided. Run `bitrub-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
