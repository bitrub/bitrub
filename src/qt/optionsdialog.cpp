    ...
    mapper(nullptr)
{
    ...
    ui->bitrubAtStartup->setVisible(false);
    ui->verticalLayout_Main->removeWidget(ui->bitrubAtStartup);
    ...
    ui->bitrubAtStartup->setToolTip(ui->bitrubAtStartup->toolTip().arg(PACKAGE_NAME));
    ui->bitrubAtStartup->setText(ui->bitrubAtStartup->text().arg(PACKAGE_NAME));

    ui->openBitRubConfButton->setToolTip(ui->openBitRubConfButton->toolTip().arg(PACKAGE_NAME));
    ...
    ui->unit->setModel(new BitRubUnits(this));
...
{
    /* Main */
    mapper->addMapping(ui->bitrubAtStartup, OptionsModel::StartAtStartup);
...
void OptionsDialog::on_openBitRubConfButton_clicked()
{
