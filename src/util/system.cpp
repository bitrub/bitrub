...
const char * const BITRUB_CONF_FILENAME = "bitrub.conf";
 ...
 * https://github.com/bitrub/bitrub/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitrub-tx using stdin
        std::string val;
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitRub
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitRub
    // Mac: ~/Library/Application Support/BitRub
    // Unix: ~/.bitrub
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitRub";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitRub";
#else
    // Unix
    return pathRet / ".bitrub";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITRUB_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure BitRub Core copyright is not removed by accident
    if (copyright_devs.find("BitRub Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The BitRub Core developers";
    }
