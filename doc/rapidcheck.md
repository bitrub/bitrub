# RapidCheck property-based testing for BitRub Core

## Concept

Property-based testing is experimentally being added to BitRub Core with
...
If RapidCheck is installed, BitRub Core will automatically run the
...
3. Configure BitRub Core with RapidCheck.

    `cd` to the directory of your local bitrub repository and run
...
4. Build BitRub Core with RapidCheck.
    ...
    Making all in src
    [...]
    CXX      test/gen/test_bitrub-crypto_gen.o
    CXX      test/test_bitrub-key_properties.o
    ```
...
That's it! You are now running property-based tests in BitRub Core.
